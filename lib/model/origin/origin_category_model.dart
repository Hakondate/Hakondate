...
abstract class OriginCategoryModel with _$OriginCategoryModel {
  const factory OriginCategoryModel({required String name, required List<OriginItemModel> items}) = _OriginCategoryModel;
  ...
  factory OriginCategoryModel.fromFirestore(MapEntry<String, dynamic> category) {
    ...
    return OriginCategoryModel(name: category.key, items: items.entries.map(OriginItemModel.fromFirestore).toList());
  }

  Map<String, Object> toFirestore() => <String, Object>{name: items.map((OriginItemModel item) => item.toFirestore()).toList()};
}
